...
function spanNamed(spans: SpySpan[], name: string) {
...
describe("tool spans", () => {
  test("starts a tool span on running status", () => {
    ...
    const span = spanNamed(tracer.spans, "opencode.tool.bash");
  ...
  test("tool span carries tool.name attribute", () => {
    ...
    const span = spanNamed(tracer.spans, "opencode.tool.read_file");
    expect(span.attributes["tool.name"]).toBe("read_file");
    expect(span.attributes[TOOL_NAME]).toBe("read_file");
    expect(span.attributes[OPENINFERENCE_SPAN_KIND]).toBe(
  ...
  test("tool span result_size_bytes matches exact byte length of multibyte output", () => {
    ...
    expect(
      spanNamed(tracer.spans, "opencode.tool.bash").attributes[
  ...
  test("tool span error attr set on error status", () => {
    ...
    handleMessagePartUpdated(makeToolPartUpdated("error"), ctx);
    expect(
      spanNamed(tracer.spans, "opencode.tool.bash").attributes["tool.error"],
    ).toBe("fail");
  });
...
describe("message (LLM) spans", () => {
  test("startMessageSpan creates an llm span", () => {
    ...
    expect(spanNamed(tracer.spans, "opencode.llm").name).toBe("opencode.llm");
  ...
  test("startMessageSpan sets OpenInference LLM attributes", () => {
    ...
    const span = spanNamed(tracer.spans, "opencode.llm");
    expect(span.attributes[OPENINFERENCE_SPAN_KIND]).toBe(
      OpenInferenceSpanKind.LLM,
    );
    expect(span.attributes[LLM_SYSTEM]).toBe("openai");
    expect(span.attributes[LLM_PROVIDER]).toBe("openai");
    expect(span.attributes[LLM_MODEL_NAME]).toBe("gpt-4o");
  });
  ...
  test("startMessageSpan is a no-op when span already exists for sessionID:messageID", () => {
    ...
    startMessageSpan("ses_1", "msg_1", "claude", "anthropic", 1000, ctx);
    expect(
      tracer.spans.filter((span) => span.name === "opencode.llm"),
    ).toHaveLength(1);
  });
  ...
  test("handleMessageUpdated sets OK status on success", () => {
    ...
    expect(spanNamed(tracer.spans, "opencode.llm").status.code).toBe(
      SpanStatusCode.OK,
    );
  });
...
describe("OPENCODE_DISABLE_TRACES=session", () => {
  ...
  test("session.created log record still emitted", () => {
    ...
    handleSessionCreated(makeSessionCreated("ses_1"), ctx);
    expect(
      logger.records.find((r) => r.body === "session.created"),
    ).toBeDefined();
  });
...
describe("OPENCODE_DISABLE_TRACES=llm", () => {
  ...
  test("cost counter metric still fires", () => {
    const { ctx, counters } = makeCtx("proj_test", [], ["llm"]);
    handleMessageUpdated(
      makeAssistantMessageUpdated({ id: "msg_1", cost: 0.05 }),
      ctx,
    );
...
describe("OPENCODE_DISABLE_TRACES=tool", () => {
  ...
  test("tool.duration histogram still records on completion", () => {
    ...
    handleMessagePartUpdated(
      makeToolPartUpdated("completed", { startMs: 1000, endMs: 1500 }),
      ctx,
    );
